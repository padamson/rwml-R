...
This notebook contains R code to accompany Chapter 3 of the book 
...
Figures 3.7-3.9 of the book illustrate results from analysis using linear 
regression and a support vector machine classifier with a nonlinear kernel
by plotting decision boundaries. We go "off-script" here and instead 
...
and 
[`AppliedPredictiveModeling`](https://cran.r-project.org/web/packages/AppliedPredictiveModeling/index.html) 
packages.

```{r featurePlots, fig.width=8, fig.height=8, cache=TRUE, dependson="figure3.6"}
...
As alluded to above, when we make a 80/20% train/test split, 
we use the `Survived` factor
...
Next we apply four different models to our data and print out the
associated confusion matrix and statistics for each.
...
Given these models, we can explore their performance differences by collecting 
the resampling results using `resamples` and 
visualizing the resampling distributions with `bwplot`. More info can be
found [here](http://topepo.github.io/caret/model-training-and-tuning.html#exploring-and-comparing-resampling-distributions).
