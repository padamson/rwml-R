...
*NOTE: Depending on the speed of your internet connection and computer, downloading and decompressing the archives will take a while. If you are on a really slow internet connection, the `timeout` option may need to be increased.*

```{r getData, cache=FALSE}
baseUrl <- "https://archive.org/download/nycTaxiTripData2013/"
tripFile <- "trip_data.7z"
data_dir <- "../proprietary_data"
localTripFile <- paste0(data_dir, "/", tripFile)
tripFile1 <- paste0(data_dir, "/trip_data_1.csv")
fareFile <- "trip_fare.7z"
localFareFile <- paste0(data_dir, "/", fareFile)
fareFile1 <- paste0(data_dir, "/trip_fare_1.csv")
op <- options()
options(timeout = 300)
...
if(!file.exists(localTripFile)){
  ...
  if(!file.exists(tripFile1)){
    system(paste0("7z x ", localTripFile, " -o", data_dir))
  }
}
if(!file.exists(localFareFile)){
  ...
  if(!file.exists(fareFile1)){
    system(paste0("7z x ", localFareFile, " -o", data_dir))
  }
}
options(op)
```
